...
function AuthPage() {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-background px-5 py-10" style={{ background: "var(--gradient-warm)" }}>
      <div className="w-full max-w-md rounded-3xl bg-card border border-border p-8 shadow-[var(--shadow-soft)]">
        ...
        <button
          type="button"
          onClick={google}
          className="w-full mb-4 rounded-full border border-border bg-background py-3 text-sm font-medium hover:bg-secondary transition flex items-center justify-center gap-2"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === "signup" && (
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm" />
          )}
